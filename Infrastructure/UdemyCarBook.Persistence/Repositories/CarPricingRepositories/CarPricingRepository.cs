...
/*List<CarPricing> values = new List<CarPricing>();
           using (var command = _context.Database.GetDbConnection().CreateCommand())
           {
                command.CommandText = "Select * From(Select  Model, PricingID, Amount From CarPricings Inner Join Cars On Cars.CarID = CarPricings.CarId Inner join Brands On Brands.BrandID = Cars.BrandID) As SourceTables Pivot  (Sum(Amount) For PricingID In([2], [3], [5])) as PivotTable;";
               command.CommandType = System.Data.CommandType.Text;
               _context.Database.OpenConnection();
               using (var reader=command.ExecuteReader())
               {
